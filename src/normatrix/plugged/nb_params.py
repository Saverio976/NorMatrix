try:
    ...
    from normatrix.source.custom_regex import re_compile, re_match, re_sub
    from normatrix.source.file_parser import CFileParse
except ModuleNotFoundError:
    from src.normatrix.source.config import TypeLine
    from src.normatrix.source.custom_regex import re_compile, re_match, re_sub
    from src.normatrix.source.file_parser import CFileParse

reg = re_compile(r"^(?!.*=)(\w{1,} {0,1}){2,}\((.*?\n{0,1}){0,}?\) {0,1}\n{0,1}\{")
reg_func_name = re_compile(r"(?!.*=)(\w{1,} {0,1}){2,}")
...
def check_snake_case_function(only_decl: str, list_error: list, i: int) -> int:
    ...
    if len(new.split(" ")) < 2:
        return 0
    if re_match(new.split(" ")[1], alphabet):
        list_error.append(
