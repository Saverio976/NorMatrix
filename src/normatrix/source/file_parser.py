try:
    from normatrix.source.config import REG_TYPELINE, TypeLine
    from normatrix.source.custom_regex import re_match, re_search, re_sub
except ModuleNotFoundError:
    from src.normatrix.source.config import REG_TYPELINE, TypeLine
    from src.normatrix.source.custom_regex import re_match, re_search, re_sub
...
def get_status(lines: str) -> (TypeLine, str):
    for type_reg, regex_list in REG_TYPELINE.items():
        for regex in regex_list:
            ...
            if res and res.start() <= len(lines.split("\n")[0]):
